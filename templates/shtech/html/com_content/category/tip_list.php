...
foreach($result as $row_article) : ?>
	<?php if ($row_category->tag_id == $row_article->tag_id) : ?>
		...
		$query = $db->getQuery(true);
		$query
		->select(array('a.item_id','a.field_id', 'a.value','b.name'))
		->from($db->quoteName('#__fields_values', 'a'))
		->where($db->quoteName('a.item_id') . '= ' .$row_article->id )
		->join('INNER', $db->quoteName('#__fields', 'b') . ' ON (' . $db->quoteName('a.field_id') . ' = ' . $db->quoteName('b.id') . ')')
		->where($db->quoteName('b.state') ."= 1")
		->where($db->quoteName('b.context') ."= ".$db->quote('com_content.article'));
		$db->setQuery($query);
		$fieldslist = $db->loadObjectList();
		foreach($fieldslist as $fields_values): ?>
		<?php if ($fields_values->field_id == 14): ?>
			<?php $pdf_val =  $this->escape($fields_values->value);?>
			<?php $pdf_name =  $this->escape($fields_values->name);?>
		<?php endif ?>		
			<?php $field_val =  $this->escape($fields_values->value);?>
			<?php $field_name =  $this->escape($fields_values->name);?>	
		<?php endforeach;?>
		<li class="list-group-item">
			<div class="intro-list">
				<div class="flex-grow-1" itemprop="name">
					<a href="<?=$link;?>" class="title d-block w-100" ><?=$this->escape($row_article->title);?></a>
					<?php if ($fields_values->field_id == 19 && isset($field_val) && !empty($field_val)): ?>
					<p class="<?=$field_name ?>"><?=$field_val;?></p>
				<?php endif ?>	
			</div>
			<?php if (isset($pdf_val) && !empty($pdf_val)): ?>
			<div class="flex-shrink-1 ml-auto">
				<a href="<?=$pdf_val;?>" target="_blank" class="<?=$pdf_name; ?> d-block">
						<svg 
